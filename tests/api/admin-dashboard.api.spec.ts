...
test.describe('Dashboard API smoke @P2', () => {
  ...
  test('TC_AD_DASH_API_011 booking search is case-insensitive', async ({ request }) => {
    test.skip(!apiAvailable, 'API not available');
    const baseline = await adminGet(request, '/admin/bookings?search=BK&per_page=50');
    expect(baseline.status()).toBe(200);
    const baselineIds = ((await baseline.json()).data?.data ?? []).map((r: { id: number }) => r.id).sort();

    const lower = await adminGet(request, '/admin/bookings?search=bk&per_page=50');
