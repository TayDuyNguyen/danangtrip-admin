...
import {
  ...
  type MockTourCategoryRow,
} from '../data/tour-categories.data';
...
let dataset: MockTourCategoryRow[] = structuredClone(mockTourCategoryListRows);
let nextId = 100;

export function resetMockTourCategories() {
  dataset = structuredClone(mockTourCategoryListRows);
  nextId = 100;
...
export function setTourCategoryListFail(on = true) {
  flags.listFail = on;
}

export function setTourCategoryListEmpty(on = true) {
  flags.listEmpty = on;
}

export function setTourCategoryCreateFail(on = true) {
  flags.createFail = on;
}

export function setTourCategoryUpdateFailForId(id: number | null) {
  flags.updateFailForId = id;
}

export function setTourCategoryStatusFailForId(id: number | null) {
  flags.statusFailForId = id;
}

export function setTourCategoryDeleteFailForId(id: number | null) {
  flags.deleteFailForId = id;
}

export function setTourCategoryReorderFail(on = true) {
  flags.reorderFail = on;
}

export function getMockTourCategoryById(id: number) {
...
function isListPath(url: string) {
  return /\/admin\/tour-categories$/.test(pathname(url));
}

function isReorderPath(url: string) {
  return /\/admin\/tour-categories\/reorder$/.test(pathname(url));
}

function parseCategoryId(path: string): number | null {
  const m = path.match(/\/admin\/tour-categories\/(\d+)(?:\/|$)/);
  return m ? Number(m[1]) : null;
}

function isStatusPath(url: string) {
  return /\/admin\/tour-categories\/\d+\/status$/.test(pathname(url));
}

function isDetailPath(url: string) {
  const path = pathname(url);
  return /\/admin\/tour-categories\/\d+$/.test(path) && !path.includes('/status');
}
...
function buildListPayload(url: string) {
  ...
  return {
    categories,
    stats: {
      total_categories: dataset.length,
      active_categories: dataset.filter((r) => r.status === 'active').length,
      inactive_categories: dataset.filter((r) => r.status === 'inactive').length,
      total_tours: expectedCategoryStats.totalTours,
    },
  };
}

function mapCreateBody(body: Record<string, unknown>, id: number): MockTourCategoryRow {
...
export async function mockTourCategoriesApi(page: Page) {
  const handler = async (route: Route) => {
    const request = route.request();
    const url = request.url();
    if (!url.includes('/api/v1/admin/tour-categories')) {
    ...
    if (method === 'POST' && isListPath(url)) {
      ...
      await route.fulfill({
        status: 201,
        contentType: 'application/json',
        body: JSON.stringify(successEnvelope({ category: created }, 'Tour category created successfully')),
      });
      return;
    }
    ...
    if (method === 'PUT' && isDetailPath(url)) {
      ...
      if (!row) {
        await route.fulfill({
          status: 404,
          contentType: 'application/json',
          body: JSON.stringify(errorEnvelope('Tour category not found', 404)),
        });
        return;
      }
      ...
      const body = request.postDataJSON() as Record<string, unknown>;
      const updated: MockTourCategoryRow = {
        ...row,
      ...
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify(successEnvelope({ category: updated }, 'Tour category updated successfully')),
      });
      return;
    }
    ...
    if (method === 'PATCH' && isStatusPath(url)) {
      ...
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify(successEnvelope(null, 'Tour category status updated successfully')),
      });
      return;
    }
    ...
    if (method === 'DELETE' && isDetailPath(url)) {
      ...
      if (row.tour_count > 0) {
        await route.fulfill({
          status: 400,
          contentType: 'application/json',
          body: JSON.stringify(errorEnvelope('Cannot delete category with associated tours', 400)),
        });
        return;
      }
      ...
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify(successEnvelope(null, 'Tour category deleted successfully')),
      });
      return;
    }
  ...
  await page.route('**/api/v1/admin/tour-categories**', handler);
}
