...
const LocationReportCharts: React.FC<LocationReportChartsProps> = ({ data, isLoading }) => {
    ...
    if (isLoading || !data) {
        return (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                {[0, 1].map(i => (
                    <div key={i} className="bg-white/80 backdrop-blur-md border border-slate-100 rounded-2xl p-5 shadow-xs h-[360px] flex flex-col">
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            <div className="bg-white/80 backdrop-blur-md border border-slate-100 rounded-2xl p-5 shadow-xs flex flex-col justify-between h-[360px]">
                <div>
                    <h3 className="text-sm font-black text-slate-700 leading-tight">{t('charts.category_title')}</h3>
                    <p className="text-xs font-bold text-slate-400 mt-0.5">{t('charts.category_subtitle')}</p>
                </div>
                        ...
                        <>
                            ...
                            <div className="flex-1 flex flex-col justify-center gap-1.5 self-stretch overflow-y-auto pr-1 max-h-[220px] md:max-h-full">
                                {categories.slice(0, 5).map((item, index) => {
                                    ...
                                    return (
                                        <div key={item.name} className="flex justify-between items-center text-xs font-bold text-slate-500 py-1 border-b border-slate-50/50">
                                            <div className="flex items-center gap-2 truncate">
                                                <span className="w-2.5 h-2.5 rounded-full shrink-0" style={{ backgroundColor: color }}></span>
                                                <span className="truncate max-w-[120px] md:max-w-[150px]">{item.name}</span>
                                            </div>
                            ...
                            <div className="relative w-[180px] h-[180px] shrink-0 flex items-center justify-center">
                                ...
                                <div className="absolute flex flex-col items-center justify-center select-none text-center px-4 w-full">
                                    <span className="text-xl font-black text-slate-800 leading-none truncate max-w-full">
                                        {totalLocations}
                                    </span>
                                    <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-1">LOCATIONS</span>
                                </div>
                            </div>
                        </>
                    )}
            ...
            <div className="bg-white/80 backdrop-blur-md border border-slate-100 rounded-2xl p-5 shadow-xs flex flex-col justify-between h-[360px]">
                <div>
                    <h3 className="text-sm font-black text-slate-700 leading-tight">{t('charts.district_title')}</h3>
                    <p className="text-xs font-bold text-slate-400 mt-0.5">{t('charts.district_subtitle')}</p>
                </div>
