...
import { FiImage, FiMapPin, FiMaximize2, FiStar, FiX } from 'react-icons/fi';
                ...
                <div className="relative aspect-video w-full overflow-hidden bg-slate-100">
                    ...
                    {selectedImage && (
                        <button
                            ...
                            aria-label={t('detail.hero.view_full')}
                            className="absolute top-6 right-6 p-3 rounded-2xl bg-slate-900/40 text-white backdrop-blur-sm border border-white/20 hover:bg-slate-900/60 transition-all opacity-0 group-hover:opacity-100 transform translate-y-2 group-hover:translate-y-0 focus:opacity-100"
                        >
                            <FiMaximize2 size={20} />
                        </button>
                    )}
            ...
            {isLightboxOpen && selectedImage && (
                ...
                >
                    <button
                        type="button"
                        onClick={() => setIsLightboxOpen(false)}
                        aria-label={t('detail.hero.close_lightbox')}
                        className="absolute top-6 right-6 p-3 rounded-2xl bg-white/10 text-white hover:bg-white/20 transition-colors"
                    >
                        <FiX size={24} />
                    </button>
                    <img
