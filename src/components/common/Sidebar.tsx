        ...
        <aside className="w-72 bg-slate-900 border-r border-slate-800 flex flex-col h-screen sticky top-0 transition-all duration-300 group overflow-hidden">
            ...
            <div className="p-6 border-b border-slate-800 flex items-center justify-between">
                <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-linear-to-br from-blue-500 to-indigo-600 rounded-xl flex items-center justify-center shadow-lg shadow-blue-900/40">
                        <MapPin size={22} className="text-white fill-white/20" />
                    </div>
                    <div>
                        <h1 className="text-white font-black text-lg leading-none tracking-tight">DaNangTrip</h1>
                        <p className="text-slate-400 text-[10px] mt-1 font-bold uppercase tracking-widest">{t('sidebar.system')}</p>
                    </div>
                </div>
            </div>
                            ...
                            <div key={item.path} className="flex flex-col">
                                {hasSub ? (
                                    <button
                                        ...
                                        className={`
                                            flex items-center justify-between w-full px-4 py-3 rounded-xl transition-all duration-200 group/nav
                                            ${isMainActive
                                    ...
                                    >
                                        <div className="flex items-center gap-3">
                                            <item.icon size={20} className={isMainActive ? "text-blue-500" : ""} />
                                            <span className="font-bold text-[14px]">{t(item.label)}</span>
                                        </div>
                                        <ChevronDown size={16} className={`transition-transform duration-200 ${isOpen ? 'rotate-180 text-white' : ''}`} />
                                    </button>
                                ) : (
                                    <NavLink
                                        to={item.path}
